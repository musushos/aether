...
void init_baked_points(void) {
	baked_points_move = static_cast<struct dvec2 *>(calloc(BAKED_POINTS_COUNT, sizeof(*baked_points_move)));
	baked_points_open = static_cast<struct dvec2 *>(calloc(BAKED_POINTS_COUNT, sizeof(*baked_points_open)));
	baked_points_tag = static_cast<struct dvec2 *>(calloc(BAKED_POINTS_COUNT, sizeof(*baked_points_tag)));
	baked_points_close = static_cast<struct dvec2 *>(calloc(BAKED_POINTS_COUNT, sizeof(*baked_points_close)));
	baked_points_focus = static_cast<struct dvec2 *>(calloc(BAKED_POINTS_COUNT, sizeof(*baked_points_focus)));
	baked_points_opafadein = static_cast<struct dvec2 *>(calloc(BAKED_POINTS_COUNT, sizeof(*baked_points_opafadein)));
	baked_points_opafadeout = static_cast<struct dvec2 *>(calloc(BAKED_POINTS_COUNT, sizeof(*baked_points_opafadeout)));
...
static bool scene_node_snapshot(struct wlr_scene_node *node, int32_t lx,
	...
	struct wlr_scene_node *snapshot_node = nullptr;
	switch (node->type) {
	...
	case WLR_SCENE_NODE_BUFFER: {
		...
		struct wlr_scene_buffer *snapshot_buffer =
			wlr_scene_buffer_create(snapshot_tree, nullptr);
		...
		if (scene_surface != nullptr && scene_surface->surface->buffer != nullptr) {
