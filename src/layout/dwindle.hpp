...
static DwindleNode *dwindle_locked_h_node = nullptr;
static DwindleNode *dwindle_locked_v_node = nullptr;

static DwindleNode *dwindle_new_leaf(Client *c) {
	DwindleNode *n = static_cast<DwindleNode *>(calloc(1, sizeof(DwindleNode)));
	n->client = c;
	return n;
}
...
static DwindleNode *dwindle_find_leaf(DwindleNode *node, Client *c) {
	if (!node)
		return nullptr;
	if (!node->is_split)
		return node->client == c ? node : nullptr;
...
static void dwindle_insert(DwindleNode **root, Client *new_c, Client *focused,
	...
	DwindleNode *target = focused ? dwindle_find_leaf(*root, focused) : nullptr;
	if (!target)
		target = dwindle_first_leaf(*root);

	DwindleNode *split = static_cast<DwindleNode *>(calloc(1, sizeof(DwindleNode)));
	split->is_split = true;
	split->ratio = ratio;
...
static void dwindle_resize_client(Monitor *m, Client *c) {
	...
	if (!start_drag_window) {
		...
		dwindle_locked_h_node = nullptr;
		dwindle_locked_v_node = nullptr;
...
static void dwindle_resize_client_step(Monitor *m, Client *c, int32_t dx,
	...
	DwindleNode *h_node = nullptr;
	DwindleNode *v_node = nullptr;
	DwindleNode *node = leaf->parent;
