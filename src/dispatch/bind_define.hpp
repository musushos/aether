		...
		if (grabc->isfloating) {
			...
			if (config.drag_warp_cursor) {
				...
				grabcy = rzcorner & 2 ? grabc->geom.y + grabc->geom.height
									  : grabc->geom.y;
				wlr_cursor_warp_closest(cursor, nullptr, grabcx, grabcy);
			}
...
int32_t set_proportion(const Arg *arg) {
	...
	struct ScrollerStackNode *node = nullptr;
...
int32_t spawn_shell(const Arg *arg) {
	...
	if (fork() == 0) {
		...
		execlp("sh", "sh", "-c", arg->v, (char *)nullptr);

		// fallback to bash
		execlp("bash", "bash", "-c", arg->v, (char *)nullptr);
...
int32_t switch_keyboard_layout(const Arg *arg) {
	...
	wl_list_for_each(id, &inputdevices, link) {
		...
		struct wlr_keyboard *tkb = static_cast<struct wlr_keyboard *>(id->device_data);
...
int32_t switch_layout(const Arg *arg) {
	...
	int32_t jk, ji;
	char *target_layout_name = nullptr;
	uint32_t len;
...
int32_t tagmon(const Arg *arg) {
	Monitor *m = nullptr, *cm = nullptr, *oldmon = nullptr;
	if (!selmon)
		return 0;
...
int32_t zoom(const Arg *arg) {
	Client *c = nullptr, *sel = focustop(selmon);
...
int32_t scroller_apply_stack(Client *c, Client *target_client,
	...
	struct ScrollerStackNode *tnode =
		target_client ? find_scroller_node(st, target_client) : nullptr;
