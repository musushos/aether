...
/*
 * Attempt to consolidate unavoidable suck into one file, away from aether.c.  This
...
inline int32_t client_is_x11(Client *c) {
#ifdef XWAYLAND
	return c->type == X11;
#endif
	return 0;
...
inline struct wlr_surface *client_surface(Client *c) {
#ifdef XWAYLAND
	if (client_is_x11(c))
...
inline int32_t toplevel_from_wlr_surface(struct wlr_surface *s,
										 Client **pc,
										 LayerSurface **pl) {
	...
	if ((xsurface = wlr_xwayland_surface_try_from_wlr_surface(root_surface))) {
		c = static_cast<Client *>(xsurface->data);
		type = c->type;
		goto end;
	}
#endif
	...
	if ((layer_surface =
		...
		l = static_cast<LayerSurface *>(layer_surface->data);
		type = LayerShell;
		goto end;
	}
	...
	while (xdg_surface) {
		...
		case WLR_XDG_SURFACE_ROLE_TOPLEVEL:
			c = static_cast<Client *>(xdg_surface->data);
			type = c->type;
			goto end;
...
/* The others */
inline void client_activate_surface(struct wlr_surface *s,
									int32_t activated) {
...
inline const char *client_get_appid(Client *c) {
#ifdef XWAYLAND
	if (client_is_x11(c))
...
inline int32_t client_get_pid(Client *c) {
	pid_t pid;
#ifdef XWAYLAND
	if (client_is_x11(c))
...
inline void client_get_clip(Client *c, struct wlr_box *clip) {
	clip->x = 0;
	clip->y = 0;
	clip->width = c->geom.width - 2 * c->bw;
	clip->height = c->geom.height - 2 * c->bw;
...
inline void client_get_geometry(Client *c, struct wlr_box *geom) {
#ifdef XWAYLAND
	if (client_is_x11(c)) {
...
inline Client *client_get_parent(Client *c) {
	Client *p = nullptr;
#ifdef XWAYLAND
	if (client_is_x11(c)) {
	...
	if (c->surface.xdg->toplevel->parent)
		toplevel_from_wlr_surface(
			c->surface.xdg->toplevel->parent->base->surface, &p, nullptr);
	return p;
}

inline int32_t client_has_children(Client *c) {
#ifdef XWAYLAND
	if (client_is_x11(c))
...
inline const char *client_get_title(Client *c) {
#ifdef XWAYLAND
	if (client_is_x11(c))
...
inline int32_t client_is_float_type(Client *c) {
	struct wlr_xdg_toplevel *toplevel;
...
inline int32_t client_is_rendered_on_mon(Client *c, Monitor *m) {
...
inline int32_t client_is_unmanaged(Client *c) {
#ifdef XWAYLAND
	if (client_is_x11(c))
...
inline void client_notify_enter(struct wlr_surface *s,
...
inline void client_send_close(Client *c) {
#ifdef XWAYLAND
	if (client_is_x11(c)) {
...
inline void client_set_border_color(Client *c, const float color[4]) {
	wlr_scene_rect_set_color(c->border, color);
}

inline void client_set_fullscreen(Client *c, int32_t fullscreen) {
#ifdef XWAYLAND
	if (client_is_x11(c)) {
...
inline void client_set_scale(struct wlr_surface *s, float scale) {
	wlr_fractional_scale_v1_notify_scale(s, scale);
	wlr_surface_set_preferred_buffer_scale(s, (int32_t)ceilf(scale));
}

inline uint32_t client_set_size(Client *c, uint32_t width, uint32_t height) {
#ifdef XWAYLAND
	if (client_is_x11(c)) {
		struct wlr_xwayland_surface *surface = c->surface.xwayland;
		...
		xcb_size_hints_t *size_hints = surface->size_hints;
		/* Use distinct names to avoid shadowing the function parameters */
		int32_t w = c->geom.width - 2 * c->bw;
		int32_t h = c->geom.height - 2 * c->bw;
...
inline void client_set_minimized(Client *c, bool minimized) {
#ifdef XWAYLAND
	if (client_is_x11(c)) {
...
inline void client_set_maximized(Client *c, bool maximized) {
	struct wlr_xdg_toplevel *toplevel;
...
inline void client_set_tiled(Client *c, uint32_t edges) {
	struct wlr_xdg_toplevel *toplevel;
#ifdef XWAYLAND
...
inline void client_set_suspended(Client *c, int32_t suspended) {
#ifdef XWAYLAND
	if (client_is_x11(c))
		return;
#endif
...
inline int32_t client_should_ignore_focus(Client *c) {
...
inline int32_t client_is_x11_popup(Client *c) {
...
inline int32_t client_should_global(Client *c) {
...
inline int32_t client_should_overtop(Client *c) {
...
inline int32_t client_wants_focus(Client *c) {
#ifdef XWAYLAND
...
inline int32_t client_wants_fullscreen(Client *c) {
#ifdef XWAYLAND
	if (client_is_x11(c))
...
inline bool client_request_minimize(Client *c, void *data) {
...
inline bool client_request_maximize(Client *c, void *data) {
...
inline void client_set_size_bound(Client *c) {
	struct wlr_xdg_toplevel *toplevel;
	struct wlr_xdg_toplevel_state state;
