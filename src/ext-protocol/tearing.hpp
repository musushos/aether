...
static void handle_controller_set_hint(struct wl_listener *listener, void *data) {
	struct tearing_controller *controller =
		wl_container_of(listener, controller, set_hint);
	Client *c = nullptr;

	toplevel_from_wlr_surface(controller->tearing_control->surface, &c, nullptr);
...
static void handle_controller_destroy(struct wl_listener *listener, void *data) {
	struct tearing_controller *controller =
...
void handle_tearing_new_object(struct wl_listener *listener, void *data) {
	struct wlr_tearing_control_v1 *new_tearing_control = static_cast<struct wlr_tearing_control_v1 *>(data);
	...
	struct tearing_controller *controller =
		static_cast<struct tearing_controller *>(ecalloc(1, sizeof(struct tearing_controller)));
...
bool custom_wlr_scene_output_commit(struct wlr_scene_output *scene_output,
	...
	Monitor *m = static_cast<Monitor *>(wlr_output->data);
...
void apply_tear_state(Monitor *m) {
	if (wlr_scene_output_needs_frame(m->scene_output)) {
		wlr_output_state_init(&m->pending);
		if (wlr_scene_output_build_state(m->scene_output, &m->pending, nullptr)) {
