...
static void handle_ext_commit(struct wl_listener *listener, void *data) {
	struct wlr_ext_workspace_v1_commit_event *event = 
		static_cast<struct wlr_ext_workspace_v1_commit_event *>(data);
...
static const char *get_name_from_tag(uint32_t tag) {
	static const char *names[] = {"overview", "1", "2", "3", "4",
								  "5",		  "6", "7", "8", "9"};
	return (tag < sizeof(names) / sizeof(names[0])) ? names[tag] : nullptr;
}
...
static void add_workspace_by_tag(int32_t tag, Monitor *m) {
	...
	struct workspace *workspace = static_cast<struct workspace *>(ecalloc(1, sizeof(*workspace)));
