	...
	union {
		struct {
			...
			struct wlr_ext_workspace_group_handle_v1
				*group; // nullptr if destroyed
		} create_workspace;
		struct {
			struct wlr_ext_workspace_handle_v1 *workspace; // nullptr if destroyed
		} activate;
		struct {
			struct wlr_ext_workspace_handle_v1 *workspace; // nullptr if destroyed
		} deactivate;
...
struct wlr_ext_workspace_handle_v1 {
	struct wlr_ext_workspace_manager_v1 *manager;
	struct wlr_ext_workspace_group_handle_v1 *group; // May be nullptr
	char *id;
	char *name;
