...
void aether_ipc_manager_bind(struct wl_client *client, void *data,
	...
	wl_resource_set_implementation(manager_resource,
								   &aether_manager_implementation, nullptr,
...
void aether_ipc_manager_get_output(struct wl_client *client,
	...
	if (!op)
		return;
	Monitor *monitor = static_cast<Monitor *>(op->data);
	...
	ipc_output = static_cast<aetherIpcOutput *>(ecalloc(1, sizeof(*ipc_output)));
...
static void aether_ipc_output_destroy(struct wl_resource *resource) {
	aetherIpcOutput *ipc_output = static_cast<aetherIpcOutput *>(wl_resource_get_user_data(resource));
...
void aether_ipc_output_printstatus_to(aetherIpcOutput *ipc_output) {
	...
	Client *c = nullptr, *focused = nullptr;
...
void aether_ipc_output_set_client_tags(struct wl_client *client,
	...
	ipc_output = static_cast<aetherIpcOutput *>(wl_resource_get_user_data(resource));
	if (!ipc_output)
		return;
...
void aether_ipc_output_dispatch(struct wl_client *client,
	...
	if (arg.v)
		free(const_cast<void*>(arg.v));
	if (arg.v2)
		free(const_cast<void*>(arg.v2));
	if (arg.v3)
		free(const_cast<void*>(arg.v3));
}
