...
Monitor *dirtomon(enum wlr_direction dir) {
	...
	if ((next = wlr_output_layout_adjacent_output(output_layout, 1 << dir,
		...
		return static_cast<Monitor *>(next->data);
...
Monitor *xytomon(double x, double y) {
	...
	return o ? static_cast<Monitor *>(o->data) : nullptr;
}

Monitor *get_monitor_nearest_to(int32_t lx, int32_t ly) {
	double closest_x, closest_y;
	wlr_output_layout_closest_point(output_layout, nullptr, lx, ly, &closest_x,
									&closest_y);
...
bool match_monitor_spec(char *spec, Monitor *m) {
	...
	char *name_rule = nullptr;
	char *make_rule = nullptr;
	char *model_rule = nullptr;
	char *serial_rule = nullptr;
