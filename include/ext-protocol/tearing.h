...
bool custom_wlr_scene_output_commit(struct wlr_scene_output *scene_output,
	...
	// If tearing page turning is enabled but submission fails, try again to disable tearing page turning.
	...
	if (committed) {
		...
		// Even if the submission fails, clean up the status to avoid accumulation
		if (state == &m->pending) {
			wlr_output_state_finish(&m->pending);
			wlr_output_state_init(&m->pending);
		}
