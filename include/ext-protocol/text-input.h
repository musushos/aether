...
struct aether_input_method_relay {
...
struct aether_input_method_popup {
	struct wlr_input_popup_surface_v2 *popup_surface;
	struct wlr_scene_tree *tree;
	struct wlr_scene_tree *scene_surface;
	struct aether_input_method_relay *relay;
	struct wl_list link;
...
bool aether_im_keyboard_grab_forward_modifiers(KeyboardGroup *keyboard);

struct aether_input_method_relay *aether_im_relay_create();

void aether_im_relay_finish(struct aether_input_method_relay *relay);

void aether_im_relay_set_focus(struct aether_input_method_relay *relay,
...
get_keyboard_grab(KeyboardGroup *keyboard) {
	struct wlr_input_method_v2 *input_method =
		aether_input_method_relay->input_method;
...
get_active_text_input(struct aether_input_method_relay *relay) {
...
static void update_active_text_input(struct aether_input_method_relay *relay) {
...
static void
update_text_inputs_focused_surface(struct aether_input_method_relay *relay) {
	struct text_input *text_input;
	wl_list_for_each(text_input, &relay->text_inputs, link) {
...
static void update_popup_position(struct aether_input_method_popup *popup) {
	struct aether_input_method_relay *relay = popup->relay;
...
static void update_popups_position(struct aether_input_method_relay *relay) {
	struct aether_input_method_popup *popup;
...
static void handle_input_method_new_popup_surface(struct wl_listener *listener,
	...
	struct aether_input_method_popup *popup =
		ecalloc(1, sizeof(struct aether_input_method_popup));
...
static void send_state_to_input_method(struct aether_input_method_relay *relay) {
...
static void handle_text_input_enable(struct wl_listener *listener, void *data) {
	...
	struct aether_input_method_relay *relay = text_input->relay;
...
static void handle_focused_surface_destroy(struct wl_listener *listener,
	...
	aether_im_relay_set_focus(relay, NULL);
}

struct aether_input_method_relay *aether_im_relay_create() {
	struct aether_input_method_relay *relay =
		ecalloc(1, sizeof(struct aether_input_method_relay));
